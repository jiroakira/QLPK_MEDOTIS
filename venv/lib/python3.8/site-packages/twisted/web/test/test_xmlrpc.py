...
class XMLRPCTests(unittest.TestCase):
    ...
    def proxy(self, factory=None):
        ...
        """
        p = xmlrpc.Proxy(networkString("http://127.0.0.1:8000:%d/" % self.port))
        if factory is None:
    ...
    def test_errorGet(self):
        ...
        d = agent.request(b"GET", networkString("http://127.0.0.1:8000:%d/" % (self.port,)))
    ...
    def test_errorXMLContent(self):
        ...
        agent = client.Agent(reactor)
        d = agent.request(
            uri=networkString("http://127.0.0.1:8000:%d/" % (self.port,)),
            method=b"POST",
    ...
    def test_tcpTimeout(self):
        ...
        proxy = xmlrpc.Proxy(b"http://127.0.0.1:8000:69", connectTimeout=2.0,
                             reactor=reactor)
...
class XMLRPCPublicLookupProcedureTests(unittest.TestCase):
    ...
    def createServer(self, resource):
        ...
        self.proxy = xmlrpc.Proxy(
            networkString('http://127.0.0.1:8000:%d' % self.port))
...
class SerializationConfigMixin:
    ...
    def setUp(self):
        ...
        self.port = self.p.getHost().port
        self.proxy = xmlrpc.Proxy(
            networkString("http://127.0.0.1:8000:%d/" % (self.port,)), **kwargs)
...
class XMLRPCAuthenticatedTests(XMLRPCTests):
    ...
    def test_explicitAuthInfo(self):
        p = xmlrpc.Proxy(networkString("http://127.0.0.1:8000:%d/" % (
...
class XMLRPCClientErrorHandlingTests(unittest.TestCase):
    ...
    def test_erroneousResponse(self):
        ...
        an exception.
        """
        proxy = xmlrpc.Proxy(networkString("http://127.0.0.1:8000:%d/" %
