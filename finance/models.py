...
class HoaDonThuoc(models.Model):
    ...
    * Bảng hóa đơn thuốc sẽ lưu trữ lại hóa đơn của tất cả người dùng
    ...
    """
    don_thuoc = models.OneToOneField(
        "medicine.DonThuoc", on_delete=models.CASCADE, null=True, related_name='hoa_don_thuoc')
    nguoi_thanh_toan = models.ForeignKey(
        "clinic.User", on_delete=models.SET_NULL, null=True, blank=True)
    ma_hoa_don = models.CharField(
        max_length=255, unique=True, null=True, blank=True)
    tong_tien = models.DecimalField(
        decimal_places=0, max_digits=10, null=True, blank=True)
    ...
    class Meta:
        ...
        permissions = (
            ...
            ('can_view_medicine_expense_coverage',
             'Xem danh sách thuốc bảo hiểm chi trả'),
            ('can_export_medicine_expense_coverage',
             'Xuất danh sách thuốc bảo hiểm chi trả'),
    ...
    def analysis_total_value(queryset):
        total_value = queryset.aggregate(Sum('tong_tien'))[
            'tong_tien__sum'] if queryset else 0
        return total_value
...
class HoaDonChuoiKham(models.Model):
    ...
    * Bảng hóa đơn khám sẽ lưu trữ lại hóa đơn sau khi sử dụng các dịch vụ khám của tất cả người dùng 
    ...
    chuoi_kham = models.OneToOneField(
        "clinic.ChuoiKham", on_delete=models.CASCADE, null=True, related_name='hoa_don_dich_vu')
    nguoi_thanh_toan = models.ForeignKey(
        "clinic.User", on_delete=models.SET_NULL, null=True, blank=True)
    ma_hoa_don = models.CharField(
    ...
    class Meta:
        ...
        permissions = (
            ...
            ('can_view_service_expense_coverage',
             'Xem danh sách dịch vụ kỹ thuật bảo hiểm chi trả'),
            ('can_export_service_expense_coverage',
             'Xuất danh sách dịch vụ kỹ thuật bảo hiểm chi trả'),
            ('can_view_service_revenue', 'Xem doanh thu dịch vụ kỹ thuật'),
        )

    def save(self, *args, **kwargs):
        if not self.id:
            self.thoi_gian_tao = timezone.localtime(timezone.now())
        self.thoi_gian_cap_nhat = timezone.localtime(timezone.now())
        super(HoaDonChuoiKham, self).save(*args, **kwargs)
...
class HoaDonLamSang(models.Model):
    lich_hen = models.ForeignKey("clinic.LichHenKham", on_delete=models.CASCADE,
                                 null=True, blank=True, related_name="hoa_don_lam_sang")
    nguoi_thanh_toan = models.ForeignKey(
        "clinic.User", on_delete=models.SET_NULL, null=True, blank=True)
    tong_tien = models.DecimalField(
...
class HoaDonTong(models.Model):
    benh_nhan = models.ForeignKey("clinic.User", on_delete=models.SET_NULL,
                                  null=True, blank=True, related_name="hoa_don_tong_nguoi_dung")
    lich_hen = models.ForeignKey("clinic.LichHenKham", on_delete=models.SET_NULL,
                                 null=True, blank=True, related_name="hoa_don_lich_hen")
    hoa_don_lam_sang = models.ForeignKey(
        HoaDonLamSang, on_delete=models.SET_NULL, null=True, blank=True)
    hoa_don_chuoi_kham = models.ForeignKey(
        HoaDonChuoiKham, on_delete=models.SET_NULL, null=True, blank=True)
    hoa_don_thuoc = models.ForeignKey(
        HoaDonThuoc, on_delete=models.SET_NULL, null=True, blank=True)
    bao_hiem = models.BooleanField(default=False)
...
class HoaDonVatTu(models.Model):
    ...
    class Meta:
        ...
        permissions = (
            ('can_checkout_medical_supplies_receipt',
             'Thanh toán hóa đơn vật tư y tế'),
            ...
            ('can_view_medical_supplies_expense_coverage',
             'Xem danh sách vật tư y tế bảo hiểm chi trả'),
            ('can_export_medical_supplies_expense_coverage',
             'Xuất danh sách vật tư y tế bảo hiểm chi trả'),
...
class HoaDonNhapHang(models.Model):
    nguoi_phu_trach = models.ForeignKey(
        "clinic.User", on_delete=models.CASCADE, null=True, blank=True)
    ma_hoa_don = models.CharField(
...
class NhapHang(models.Model):
    hoa_don = models.ForeignKey(HoaDonNhapHang, on_delete=models.CASCADE,
                                null=True, blank=True, related_name='hoa_don_nhap_hang')
    thuoc = models.ForeignKey("medicine.Thuoc", on_delete=models.CASCADE,
                              null=True, blank=True, related_name='nhap_hang_thuoc')
...
class TonHang(models.Model):
    hoa_don = models.ForeignKey(
        HoaDonTonHang, on_delete=models.CASCADE, null=True, blank=True)
    thuoc = models.ForeignKey(
        'medicine.Thuoc', on_delete=models.CASCADE, null=True, blank=True)
